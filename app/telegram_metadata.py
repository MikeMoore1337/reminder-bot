...
BOT_METADATA: Final = TelegramBotMetadata(
    ...
    description=(
        "Напиши обычной фразой, что и когда напомнить: если время понятно, бот сразу сохранит "
        "напоминание и покажет результат. Если нужно уточнение, он сначала спросит.\n\n"
        "Примеры:\n"
        "• напомни завтра в 9 созвон\n"
        "• напомни каждый понедельник в 10 отправить отчёт\n"
        "• напомни важное через 30 минут выключить духовку\n\n"
        "Для голосового сообщения бот покажет расшифровку и предпросмотр перед кнопкой «Создать». "
        "Для дедлайна — предпросмотр плана и явное подтверждение."
    ),
)
...
START_TEXT: Final[str] = (
    ...
    "Напиши обычной фразой, что и когда напомнить — если всё понятно, "
    "я сразу сохраню напоминание и покажу результат. Если время неоднозначно, "
    "сначала задам уточнение. Для голосовых сообщений покажу расшифровку и "
    "предпросмотр перед «Создать»; для дедлайнов — предпросмотр плана и явное "
    "подтверждение.\n\n"
...
HELP_TEXT: Final[str] = (
    ...
    "🚀 <b>Быстрый старт</b>\n"
    "Напиши обычную фразу: если время однозначно, бот сразу сохранит напоминание и покажет "
    "сохранённый результат. Если время неоднозначно, сначала задаст уточнение. Для голосового "
    "сообщения бот покажет расшифровку и предпросмотр перед кнопкой «Создать»; для /deadline — "
    "предпросмотр плана и явное подтверждение.\n\n"
    "Примеры:\n"
    "• напомни завтра в 9 созвон\n"
