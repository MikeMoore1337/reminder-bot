...
его идентификатором; если даже такой список не помещается, дайджест содержит
одну bounded-агрегацию с точным количеством и продолжением через постраничный
`/list` (`/list 2`, `/list 3` и далее).
Просроченный слот старше `DIGEST_MAX_DELAY_MINUTES` и слот
...
обновляется непосредственно перед внешним send; тот же lease token может
атомарно продлить уже истёкший timestamp, пока строка всё ещё `processing` и
не была перехвачена другим worker.
