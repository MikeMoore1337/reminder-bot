...
3. При валидном результате бот показывает фактическую строку `Распознано голосом`,
   текст, локальное время, повторение и часовой пояс с кнопками «Создать»,
   «Исправить» и «Отмена».
4. «Исправить» атомарно заменяет конкретный voice draft на существующий
   `ReminderClarification`. Пользователь отправляет одну исправленную текстовую
   команду, после чего получает новый confirmation-gated voice draft; исходная
   STT-строка сохраняется и снова показывается в preview.
5. Reminder появляется только после «Создать». Повторный или stale callback
   не создаёт вторую запись.
6. Неоднозначный или неподдержанный результат после успешного STT показывает
   фактическую расшифровку и переводит пользователя в тот же bounded
   clarification flow с сохранением voice-контекста. `/cancel` отменяет voice
   draft, clarification и другие активные сценарии.
